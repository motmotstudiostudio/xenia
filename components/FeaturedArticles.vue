  ...
  <div v-editable="blok">
    <nav class="tablehead hidden md:grid grid-cols-1 md:grid-cols-8 pb-2">
      ...
      <div
        class="col-span-1"
        @click="sort('when')"
        v-bind:class="[sortBy === 'when' ? sortDirection : '']"
      >
        When
      </div>
    </nav>
    ...
    <ul class="worklist">
      <li
        class="py-4"
        v-for="post,index in sortedArticles"
        :key="post.id"
        data-fx="xenia"
      >
        <a
...
export default {
  ...
  watch: {
    sortedArticles(newVal, oldVal) {
      // This delay fixes the issue.
      setTimeout(this.$hoverMe, 500);
    }
  }
};
</script>
